...
Use durable queues, transient exclusive queues, or durable queues with a [queue TTL](https://www.rabbitmq.com/docs/ttl#queue-ttl) instead.
...
### `rabbitmqadmin` v1 Download Endpoint is Removed

Environments that still rely on `rabbitmqadmin` v1 have two migration options:

 * Move to [`rabbitmqadmin` v2](https://www.rabbitmq.com/docs/management-cli)
 * Switch to downloading `rabbitmqadmin` v1 [from the `v4.2.x` branch](https://raw.githubusercontent.com/rabbitmq/rabbitmq-server/refs/heads/v4.2.x/deps/rabbitmq_management/bin/rabbitmqadmin) on GitHub
 ...
 * Khepri topic exchange routing projection (v4): replaced the internal representation with a trie
   ...
   GitHub issue: [#15619](https://github.com/rabbitmq/rabbitmq-server/pull/15619)

 * Quorum queues notify AMQP 1.0 clients of Single Active Consumer state changes

   GitHub issue: [#15736](https://github.com/rabbitmq/rabbitmq-server/pull/15736)
 ...
 * AMQP 1.0 sessions could grant too many credits in certain failure and recovery scenarios
   ...
   GitHub issue: [#15883](https://github.com/rabbitmq/rabbitmq-server/pull/15883)

 * Quorum queues: `acquired-count` is now correctly preserved when dead-lettering

   GitHub issue: [#16039](https://github.com/rabbitmq/rabbitmq-server/pull/16039)
 ...
 * `ra` was upgraded to [`3.1.6`](https://github.com/rabbitmq/ra/releases/tag/v3.1.6)
